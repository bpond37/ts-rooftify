...
export function* topArtistsSaga() {
  yield all([takeEvery(getTopArtists.request, getTopArtistsSaga)]);
}

export const topArtist = createReducer<
  topArtistState,
  ReturnType<typeof getTopArtists.success>
>(initialState).handleAction(
  getTopArtists.success,
  (state: topArtistState, action: { payload: payloadType }) => ({
    ...state,
