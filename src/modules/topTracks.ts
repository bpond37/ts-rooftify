...
export function* topTracksSaga() {
  yield all([takeEvery(getTopTracks.request, getTopTracksSaga)]);
}

export const topTrack = createReducer<
  topTrackState,
  ReturnType<typeof getTopTracks.success>
>(initialState).handleAction(
  getTopTracks.success,
  (state: topTrackState, action: { payload: payloadType }) => ({
    ...state,
