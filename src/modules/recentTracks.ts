...
function* getRecentTracksSaga(action : recentTrackAction):Generator{
  try {
    const response : any = yield call(api.getUserRecentlyPlayedTracks, action.payload)
...
export function* recentTrackSaga(){
  yield all([
    takeEvery(getRecentTracks.request, getRecentTracksSaga)
  ])
}

export const recentTrack = createReducer<recentTrackState, ReturnType<typeof getRecentTracks.success>>(initialState)
.handleAction(getRecentTracks.success, (state:recentTrackState, action:{payload:payloadType})=>({...state, items: action.payload}))
