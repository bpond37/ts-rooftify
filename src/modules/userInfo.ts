...
export function* userSaga(){
  yield all([
    takeEvery(getUser.request, getUserSaga)
  ]);
}

export const userInfo = createReducer<getUserState, ReturnType<typeof getUser.success>>(initialState)
.handleAction(getUser.success, (state: getUserState, action: { payload: User; }) =>({...state, user: action.payload}))
